...
/**
 * customize display of timeline in case when deal category is 0 & user department is 721 or 725
 */
$dealCategory = $arResult['EXTRAS']['CATEGORY_ID'];
$rsUser = $USER->GetByID($USER->GetID());
$arUser = $rsUser->fetch();
$arDepartments = $arUser['UF_DEPARTMENT'];
if ($dealCategory === 0) {
	foreach ($arDepartments as $department) {
		if (in_array($department, [721, 725])) {
